...
<div x-data="{
    lokasiMode: '{{ $pembangunan->id_lokasi ? 'pilih' : 'manual' }}',
    fotoPreview: null,
    fotoNama: null,

    sdOpen: false,
    sdSearch: '',
    sdSelected: window.__editSdSelected || [],
    sdAll: [
    ...
    dana_pemerintah: '{{ old('dana_pemerintah', $pembangunan->dana_pemerintah ?? '') }}',
    dana_provinsi:   '{{ old('dana_provinsi', $pembangunan->dana_provinsi ?? '') }}',
    dana_kabkota:    '{{ old('dana_kabkota', $pembangunan->dana_kabkota ?? '') }}',
    swadaya:         '{{ old('swadaya', $pembangunan->swadaya ?? '') }}',
    sumber_lain:     '{{ old('sumber_lain', $pembangunan->sumber_lain ?? '') }}',
    get paguAnggaran() {
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <span class="px-2.5 py-1 text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-full">
                Edit Data
            </span>
        </div>
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Edit Data</span>
        </nav>
    </div>
...
{{-- Pre-fill data untuk Alpine --}}
    <script>
        window.__editSdSelected = @json(
            $pembangunan->id_sumber_dana && $pembangunan->sumberDana
                ? [['value' => (string)$pembangunan->id_sumber_dana, 'label' => $pembangunan->sumberDana->nama]]
                : []
        );
    </script>

    {{-- FORM --}}
    <form method="POST" action="{{ route('admin.pembangunan-utama.update', $pembangunan) }}"
            ...
            <div class="flex-1 min-w-0 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-visible">
                ...
                <div class="divide-y divide-gray-100 dark:divide-slate-700">
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="space-y-1">
                            <input id="nama" type="text" name="nama" value="{{ old('nama', $pembangunan->nama) }}"
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="space-y-1">
                            <input id="volume" type="number" name="volume" value="{{ old('volume', $pembangunan->volume) }}"
                        ...
                        <div class="space-y-1">
                            <div class="flex gap-2">
                                ...
                                <div class="flex-1 space-y-1">
                                    <input type="number" name="waktu" placeholder="Lamanya pembangunan" min="0" value="{{ old('waktu', $pembangunan->waktu) }}"
                                        class="w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors
                                ...
                                <div class="w-36 shrink-0"
                                    x-data="{
                                        open: false,
                                        selected: '{{ old('satuan_waktu', $pembangunan->satuan_waktu ?? 'Hari') }}',
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="relative"
                                    x-data="{
                                        open: false,
                                        selected: '{{ old('tahun_anggaran', $pembangunan->tahun_anggaran ?? date('Y')) }}',
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="space-y-1">
                                <p class="text-xs font-medium text-gray-500 dark:text-slate-400">Sumber Biaya Pemerintah</p>
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="space-y-1">
                                ...
                                <input type="number" name="realisasi" placeholder="Realisasi Anggaran" min="0" value="{{ old('realisasi', $pembangunan->realisasi ?? '') }}"
                                    class="w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="space-y-1">
                            <div class="relative"
                                x-data="{
                                    open: false,
                                    selected: '{{ old('sifat_proyek', $pembangunan->sifat_proyek ?? '') }}',
                                    label: '{{ old('sifat_proyek', $pembangunan->sifat_proyek ?? '') }}',
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="space-y-1">
                            <input id="pelaksana" type="text" name="pelaksana" value="{{ old('pelaksana', $pembangunan->pelaksana ?? '') }}"
                                placeholder="Pelaksana Kegiatan Pembangunan" value="{{ old('pelaksana', $pembangunan->pelaksana ?? '') }}"
                                class="w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors
                        ...
                        <div class="space-y-3">
                            ...
                            <div x-show="lokasiMode === 'pilih'" class="space-y-1">
                                <div class="relative"
                                    x-data="{
                                        open: false,
                                        selected: '{{ old('id_lokasi', $pembangunan->id_lokasi ?? '') }}',
                                        label: '',
                            ...
                            <div x-show="lokasiMode === 'manual'" style="display:none">
                                <textarea name="lokasi_manual" rows="3" placeholder="Lokasi"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-y transition-colors">{{ old('lokasi_manual', $pembangunan->dokumentasi ?? '') }}</textarea>
                            </div>
                        ...
                        <div class="space-y-1">
                            <textarea id="manfaat" name="manfaat" rows="3" placeholder="Manfaat"
                                class="w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-y transition-colors
                                    {{ $errors->has('manfaat') ? 'border-red-400' : 'border-gray-300 dark:border-slate-600' }}">{{ old('manfaat', $pembangunan->manfaat ?? '') }}</textarea>
                            @error('manfaat')
                                <p class="text-xs text-red-500 flex items-center gap-1">
                                    <svg class="w-3 h-3 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                    {{ $message }}
                    ...
                    <div class="grid grid-cols-[220px_1fr] gap-4 px-6 py-4">
                        ...
                        <div class="space-y-1">
                            <textarea id="keterangan" name="keterangan" rows="3" placeholder="Keterangan"
                                class="w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-y transition-colors
                                    {{ $errors->has('keterangan') ? 'border-red-400' : 'border-gray-300 dark:border-slate-600' }}">{{ old('keterangan', $pembangunan->keterangan ?? '') }}</textarea>
                            @error('keterangan')
                                <p class="text-xs text-red-500 flex items-center gap-1">
                                    <svg class="w-3 h-3 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                    {{ $message }}
            ...
            <div class="w-64 shrink-0">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        @if($pembangunan->foto)
                            <div x-show="!fotoPreview" class="space-y-1">
                                <img src="{{ asset('storage/' . $pembangunan->foto) }}"
                                    alt="Foto saat ini"
                                    class="w-full aspect-[4/3] object-cover rounded-lg border border-gray-200 dark:border-slate-600">
                                <p class="text-xs text-gray-400 dark:text-slate-500 text-center">Foto saat ini (kosongkan jika tidak ingin mengubah)</p>
                            </div>
                        @endif
