...
@section('title', 'Edit Kader Pemberdayaan Masyarakat')
@section('content')

{{-- HEADER --}}
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Edit Kader Pemberdayaan Masyarakat</h2>
        <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Edit data kader pemberdayaan masyarakat</p>
    </div>
...
<form method="POST" action="{{ route('admin.buku-administrasi.pembangunan.kader-pemberdayaan.update', ['kader_pemberdayaan' => $kader->id]) }}"
      class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama" value="{{ old('nama', $kader->nama) }}" placeholder="Nama lengkap"
            ...
            <div>
                ...
                <input type="text" name="nik" value="{{ old('nik', $kader->nik) }}" maxlength="16" placeholder="16 digit NIK"
            ...
            <div>
                ...
                <div class="flex items-center gap-4 mt-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="jenis_kelamin" value="L" {{ old('jenis_kelamin', $kader->jenis_kelamin) == 'L' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="jenis_kelamin" value="P" {{ old('jenis_kelamin', $kader->jenis_kelamin) == 'P' ? 'checked' : '' }}
                               class="w-4 h-4 text-emerald-600 border-gray-300 focus:ring-emerald-500">
            ...
            <div>
                ...
                <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $kader->tempat_lahir) }}" placeholder="Kota tempat lahir"
                       class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
            </div>

            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1.5">Tanggal Lahir</label>
<input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $kader->tanggal_lahir?->format('Y-m-d')) }}"
                       class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="alamat" rows="2" placeholder="Alamat lengkap"
                          class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors resize-none">{{ old('alamat', $kader->alamat) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="pendidikan" class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
                    ...
                    <option value="SMA" {{ old('pendidikan', $kader->pendidikan) == 'SMA' ? 'selected' : '' }}>SMA/SMK</option>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="bidang_tugas" value="{{ old('bidang_tugas', $kader->bidang_tugas) }}" placeholder="Contoh: Kesehatan, Pendidikan, dll"
                       class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
            </div>
            ...
            <div>
                ...
                <select name="tahun_aktif" class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
                    ...
                    <option value="{{ $year }}" {{ old('tahun_aktif', $kader->tahun_aktif) == $year ? 'selected' : '' }}>{{ $year }}</option>
                    @endfor
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="keterangan" rows="3" placeholder="Keterangan tambahan..."
                          class="w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors resize-none">{{ old('keterangan', $kader->keterangan) }}</textarea>
            </div>
        </div>
    </div>
