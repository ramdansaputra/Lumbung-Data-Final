...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Buku Aparatur Pemerintah Desa</h2>
        <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Data perangkat dan aparatur pemerintah desa</p>
    </div>
    <div class="flex items-center gap-3">
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Aparatur</span>
        </nav>
    </div>
</div>
...
{{-- FILTER                                                       --}}
{{-- ============================================================ --}}
<form method="GET" action="{{ route('admin.buku-administrasi.umum.pemerintah.index') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="md:col-span-2">
            ...
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari nama, nip, niap..."
        ...
        {{-- Jabatan --}}
        <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1.5">Jabatan</label>
            <select name="jabatan" onchange="this.form.submit()"
                ...
                @foreach(\App\Models\PerangkatDesa::distinct()->pluck('jabatan') as $jabatan)
                    @if($jabatan)
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Nama Lengkap & NIP/NIAP</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">TTL & Jenis Kelamin</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Jabatan</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">SK Pengangkatan</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                    <td class="px-5 py-4 text-gray-400 dark:text-slate-500 font-medium">
                        {{ $pemerintah->firstItem() + $index }}
                    </td>
                    <td class="px-5 py-4">
                        <div class="font-semibold text-gray-800 dark:text-slate-200">{{ $item->nama_lengkap }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">NIP: {{ $item->nip ?? '-' }} | NIAP: {{ $item->niap ?? '-' }}</div>
                    </td>
                    <td class="px-5 py-4 hidden md:table-cell">
                        <div class="text-gray-600 dark:text-slate-400">{{ $item->tempat_lahir }}, {{ \Carbon\Carbon::parse($item->tanggal_lahir)->translatedFormat('d M Y') }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">{{ $item->jenis_kelamin }}</div>
                    </td>
                    <td class="px-5 py-4">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                            {{ $item->jabatan }}
                        </span>
                    </td>
                    <td class="px-5 py-4 hidden lg:table-cell">
                        <div class="text-gray-600 dark:text-slate-400">{{ $item->nomor_keputusan_pengangkatan }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">Tgl: {{ \Carbon\Carbon::parse($item->tanggal_keputusan_pengangkatan)->translatedFormat('d M Y') }}</div>
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.buku-administrasi.umum.pemerintah.edit', $item->id) }}"
                                title="Edit"
