...
{{-- FILTER                                                       --}}
{{-- ============================================================ --}}
<form method="GET" action="{{ route('admin.buku-administrasi.umum.tanah-kas-desa.index') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="md:col-span-2">
            ...
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari asal tanah, lokasi..."
        ...
        {{-- Jenis Tanah --}}
        <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1.5">Jenis Tanah</label>
            <select name="jenis" onchange="this.form.submit()"
                ...
                <option value="Sawah" {{ request('jenis') == 'Sawah' ? 'selected' : '' }}>Sawah</option>
                <option value="Ladang" {{ request('jenis') == 'Ladang' ? 'selected' : '' }}>Ladang</option>
                <option value="Tegalan" {{ request('jenis') == 'Tegalan' ? 'selected' : '' }}>Tegalan</option>
                <option value="Perkembangan" {{ request('jenis') == 'Perkembangan' ? 'selected' : '' }}>Perkembangan</option>
            </select>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Asal Tanah & Perolehan</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">No. Sertifikat & Luas</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Lokasi & Jenis</th>
                    <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Patok & Papan</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-5 py-4">
                        <div class="font-semibold text-gray-800 dark:text-slate-200">{{ $item->asal_tanah_kas_desa }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">{{ $item->asal_perolehan }} ({{ \Carbon\Carbon::parse($item->tanggal_perolehan)->translatedFormat('d M Y') }})</div>
                    </td>
                    <td class="px-5 py-4 hidden md:table-cell">
                        <div class="font-semibold text-gray-800 dark:text-slate-200">{{ $item->nomor_sertifikat ?? 'Belum ada SK' }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">Luas: {{ number_format($item->luas, 0, ',', '.') }} m² | Kelas: {{ $item->kelas ?? '-' }}</div>
                    </td>
                    <td class="px-5 py-4 hidden lg:table-cell">
                        <div class="text-gray-600 dark:text-slate-400">{{ $item->lokasi }}</div>
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 mt-1">
                            {{ $item->jenis_tanah }}
                        </span>
                    </td>
                    <td class="px-5 py-4 text-center hidden lg:table-cell">
                        <div class="text-xs {{ $item->status_patok == 'Ada' ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' }}">
                            Patok: {{ $item->status_patok }}
                        </div>
                        <div class="text-xs {{ $item->status_papan_nama == 'Ada' ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' }}">
                            Papan: {{ $item->status_papan_nama }}
                        </div>
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.buku-administrasi.umum.tanah-kas-desa.edit', $item->id) }}"
                                title="Edit"
