...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Buku Ekspedisi</h2>
        <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Merekod penghantaran (pengiriman) surat keluar</p>
    </div>
    <div class="flex items-center gap-3">
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Ekspedisi</span>
        </nav>
    </div>
</div>
...
<div class="flex items-center justify-end gap-2 mb-6">
    <a href="{{ route('admin.buku-administrasi.umum.ekspedisi.create') }}"
        ...
        <span class="hidden sm:inline">Tambah Data Ekspedisi</span>
    </a>
</div>
...
{{-- FILTER                                                       --}}
{{-- ============================================================ --}}
<form method="GET" action="{{ route('admin.buku-administrasi.umum.ekspedisi.index') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="flex items-end">
            <a href="{{ route('admin.buku-administrasi.umum.ekspedisi.index') }}"
...
<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
    ...
    <div class="flex flex-col items-center justify-center py-16 text-gray-400">
        <svg class="w-16 h-16 mb-4 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m3-2v4m0 0v4m0-4h.01M12 17h.01"/>
        </svg>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Tarikh Penghantaran</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">Tarikh & No. Surat</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Ditujukan Kepada</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Isi Ringkas Surat</th>
                    <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Tindakan</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-5 py-4 hidden md:table-cell">
                        <div class="font-semibold text-gray-800 dark:text-slate-200">{{ $item->nomor_surat }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">Tgl: {{ \Carbon\Carbon::parse($item->tanggal_surat)->translatedFormat('d M Y') }}</div>
                    </td>
                    <td class="px-5 py-4 font-medium text-gray-800 dark:text-slate-200">
                        {{ $item->tujuan }}
                    </td>
                    <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden lg:table-cell">
                        {{ $item->isi_singkat }}
                        @if($item->keterangan)
                        <div class="text-xs text-gray-400 dark:text-slate-500 mt-1">Ket: {{ $item->keterangan }}</div>
                        @endif
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.buku-administrasi.umum.ekspedisi.edit', $item->id) }}"
                                title="Edit"
                            ...
                            <form action="{{ route('admin.buku-administrasi.umum.ekspedisi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    title="Hapus"
