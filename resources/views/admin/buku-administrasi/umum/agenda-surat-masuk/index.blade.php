...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Buku Agenda Surat Masuk</h2>
        <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Pencatatan agenda surat yang diterima oleh desa</p>
    </div>
    <div class="flex items-center gap-3">
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Surat Masuk</span>
        </nav>
    </div>
</div>
...
<div class="flex items-center justify-end gap-2 mb-6">
    <a href="{{ route('admin.buku-administrasi.umum.agenda-surat-masuk.create') }}"
        ...
        <span class="hidden sm:inline">Tambah Surat Masuk</span>
    </a>
</div>
...
{{-- FILTER                                                       --}}
{{-- ============================================================ --}}
<form method="GET" action="{{ route('admin.buku-administrasi.umum.agenda-surat-masuk.index') }}"
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Nomor & Tanggal Surat</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">Tanggal Diterima</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Pengirim</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Isi Singkat / Disposisi</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden md:table-cell">
                        {{ \Carbon\Carbon::parse($item->tanggal_penerimaan)->translatedFormat('d F Y') }}
                    </td>
                    ...
                    <td class="px-5 py-4 text-gray-600 dark:text-slate-400 hidden lg:table-cell">
                        {{ $item->isi_singkat }}
                        @if($item->disposisi)
                        <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                            Disp: {{ $item->disposisi }}
                        </span>
                        @endif
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.buku-administrasi.umum.agenda-surat-masuk.edit', $item->id) }}"
                                title="Edit"
                            ...
                            <form action="{{ route('admin.buku-administrasi.umum.agenda-surat-masuk.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    title="Hapus"
