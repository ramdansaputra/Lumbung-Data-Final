...
{{-- FILTER                                                       --}}
{{-- ============================================================ --}}
<form method="GET" action="{{ route('admin.buku-administrasi.umum.tanah-desa.index') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="md:col-span-2">
            ...
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari nama pemilik, letak tanah..."
        ...
        {{-- Status --}}
        <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1.5">Status Hak Tanah</label>
            <select name="status" onchange="this.form.submit()"
                ...
                <option value="HM" {{ request('status') == 'HM' ? 'selected' : '' }}>HM (Hak Milik)</option>
                <option value="HGB" {{ request('status') == 'HGB' ? 'selected' : '' }}>HGB (Hak Guna Bangunan)</option>
                <option value="HP" {{ request('status') == 'HP' ? 'selected' : '' }}>HP (Hak Pakai)</option>
                <option value="HL" {{ request('status') == 'HL' ? 'selected' : '' }}>HL (Hak Lahan)</option>
            </select>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Nama Pemilik</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">Letak Tanah & Luas</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Status & Penggunaan</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Keterangan</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-5 py-4">
                        <div class="font-semibold text-gray-800 dark:text-slate-200">{{ $item->nama_pemilik }}</div>
                    </td>
                    <td class="px-5 py-4 hidden md:table-cell">
<div class="text-gray-600 dark:text-slate-400">{{ $item->letak_tanah }}</div>
                        <div class="text-xs text-gray-400 dark:text-slate-500">Luas: {{ number_format($item->luas_tanah, 0, ',', '.') }} m²</div>
                    </td>
                    <td class="px-5 py-4 hidden lg:table-cell">
                        ...
                        <div class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $item->penggunaan_tanah }}</div>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.buku-administrasi.umum.tanah-desa.edit', $item->id) }}"
                                title="Edit"
