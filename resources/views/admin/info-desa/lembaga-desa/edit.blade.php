...
<div>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Edit: {{ $lembaga->nama }}</span>
        </nav>
    </div>
    ...
    {{-- ── Form ── --}}
    <form action="{{ route('admin.lembaga-desa.update', $lembaga->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="p-4 space-y-4">
                ...
                <div id="logo-preview"
                    ...
                    @if($lembaga->logo && \Illuminate\Support\Facades\Storage::disk('public')->exists($lembaga->logo))
                        <img src="{{ asset('storage/'.$lembaga->logo) }}" class="w-full h-full object-contain p-4" alt="Current Logo">
                    @else
                        <img src="{{ asset('images/lumbung-data-logo.png') }}" alt="Default Logo" class="w-full h-full object-contain p-4">
                    @endif
                </div>
            ...
            <div class="p-4 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="kode" name="kode" value="{{ old('kode', $lembaga->kode) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="no_sk" name="no_sk" value="{{ old('no_sk', $lembaga->no_sk) }}"
                    ...
                    <div>
                        ...
                        <div class="relative" x-data="{
                            open: false,
                            search: '',
                            selected: { id: '{{ old('kategori_id', $lembaga->kategori_id) }}', label: '{{ old('kategori_id', $lembaga->kategori_id) ? ($kategoris->firstWhere('id', old('kategori_id', $lembaga->kategori_id))?->nama ?? 'Pilih Kategori') : 'Pilih Kategori' }}' },
                            options: [
                            ...
                            <input type="hidden" id="kategoriInput" name="kategori_id" value="{{ old('kategori_id', $lembaga->kategori_id) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative" x-data="{
                            open: false,
                            search: '',
                            selected: { id: '{{ old('ketua', $lembaga->ketua) }}', label: '{{ old('ketua', $lembaga->ketua) ? (explode(' - ', old('ketua', $lembaga->ketua))[1] ?? old('ketua', $lembaga->ketua)) : 'Pilih Ketua Lembaga' }}' },
                            options: [
                            ...
                            <input type="hidden" id="ketuaInput" name="ketua" value="{{ old('ketua', $lembaga->ketua) }}">
                            <div @click="open = !open" @click.outside="open = false; search = ''"
                                class="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 cursor-pointer flex items-center justify-between gap-2 focus-within:ring-2 focus-within:ring-emerald-500 hover:border-emerald-400 transition-colors">
                                <span x-text="selected.label" :class="selected.id === '' ? 'text-gray-400 dark:text-slate-500' : ''"></span>
                                <svg class="w-4 h-4 text-gray-400 flex-shrink-0 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <div class="relative" x-data="{
                            open: false,
                            search: '',
                            selected: { id: '{{ old('aktif', $lembaga->aktif) }}', label: '{{ old('aktif', $lembaga->aktif) == 1 ? 'Aktif' : (old('aktif', $lembaga->aktif) == 0 ? 'Nonaktif' : 'Pilih Status') }}' },
                            options: [
                            ...
                            <input type="hidden" id="aktifInput" name="aktif" value="{{ old('aktif', $lembaga->aktif) }}">
                            <div @click="open = !open" @click.outside="open = false; search = ''"
                                class="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 cursor-pointer flex items-center justify-between gap-2 focus-within:ring-2 focus-within:ring-emerald-500 hover:border-emerald-400 transition-colors">
                                <span x-text="selected.label" :class="selected.id === '' ? 'text-gray-400 dark:text-slate-500' : ''"></span>
                                <svg class="w-4 h-4 text-gray-400 flex-shrink-0 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4"
                        ...
                        class="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-colors resize-none">{{ old('deskripsi', $lembaga->deskripsi) }}</textarea>
                ...
                <div class="flex items-center justify-between gap-3 pt-4 border-t border-gray-100 dark:border-slate-700">
                    ...
                    <button type="submit"
                        class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium rounded-md shadow-sm transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    document.getElementById('logo')?.addEventListener('change', function (e) {
        ...
        if (file) {
            ...
            reader.onload = function (event) {
                ...
                preview.innerHTML = `<img src="${event.target.result}" class="w-full h-full object-contain p-4" alt="New Logo Preview">`;
            };
