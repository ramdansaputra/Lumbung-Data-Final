...
@section('title', 'Tambah Kategori Lembaga')
...
@section('content')
    <div>
        ...
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-bold text-gray-800 dark:text-slate-100">Tambah Kategori Lembaga</h2>
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                <p class="text-sm text-gray-500 dark:text-slate-400">Tambahkan kategori lembaga baru ke sistem</p>
            </div>

            {{-- ── Form Body ── --}}
            <form action="{{ route('admin.lembaga-kategori.store') }}" method="POST">
                @csrf
                ...
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4"
                            ...
                            class="w-full px-3 py-2 border @error('deskripsi') border-red-400 dark:border-red-500 @else border-gray-300 dark:border-slate-600 @enderror rounded-md text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-colors resize-none">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
