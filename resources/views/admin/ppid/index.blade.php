...
<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
    ...
    {{-- Baris Filter --}}
    <div class="px-5 pb-4">
        <form method="GET" action="{{ route('admin.ppid.index') }}" id="form-filter" class="flex flex-wrap items-center gap-2">
            ...
            <input type="hidden" name="jenis_dokumen" id="val-jenis_dokumen" value="{{ request('jenis_dokumen') }}">
            ...
            <div class="relative w-36"
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="p-2 border-b border-gray-100 dark:border-slate-700">
                        <input type="text" x-model="search" x-ref="searchTahun"
            ...
            <div class="relative w-52"
                ...
                <button type="button" @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-2 border rounded-lg text-sm cursor-pointer
                           ...
                           hover:border-emerald-400 dark:hover:border-emerald-500 transition-colors"
                    :class="open ? 'border-emerald-500 ring-2 ring-emerald-500/20' : ''">
                    <span class="truncate" x-text="label || placeholder" :class="label ? '' : 'text-gray-400 dark:text-slate-500'"></span>
                    <svg class="w-4 h-4 text-gray-400 flex-shrink-0 ml-1 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-gray-200 dark:border-slate-700">
        ...
        {{-- Tampilkan X entri --}}
        <form method="GET" action="{{ route('admin.ppid.index') }}" class="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400">
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                    ...
                    <th class="px-4 py-4 text-left text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider">WAKTU RETENSI DOKUMEN</th>
                ...
                @forelse($dokumen as $item)
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors"
                        ...
                        <td class="px-4 py-4 text-sm text-gray-600 dark:text-slate-400 whitespace-nowrap">
                            @php
                                $exp = null;
                                if ($item->tanggal_terbit && $item->waktu_retensi && $item->waktu_retensi !== 'Permanen') {
