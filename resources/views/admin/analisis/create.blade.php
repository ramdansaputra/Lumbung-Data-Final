...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Tambah Analisis</h2>
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Buat master analisis data potensi/sumber daya baru</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-slate-500">Tambah Data Baru</p>
            </div>
        </div>

        <form action="{{ route('admin.analisis.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama') }}"
                    placeholder="Contoh: Analisis Kemiskinan 2025"
            ...
            <div>
                ...
                <select name="subjek"
                    ...
                    <option value="RUMAH_TANGGA" {{ old('subjek') === 'RUMAH_TANGGA' ? 'selected' : '' }}>Rumah Tangga</option>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="AKTIF"       {{ old('status', 'AKTIF') === 'AKTIF'       ? 'selected' : '' }}>Tidak Terkunci (Aktif)</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    @error('kode')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Dikosongkan = otomatis dibuat dari nama</p>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Tahun Periode</label>
                    <input type="number" name="periode" value="{{ old('periode', date('Y')) }}"
                        min="2000" max="2100"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                    placeholder="Deskripsi singkat tentang analisis ini..."
                    class="w-full px-4 py-2.5 text-sm border border-gray-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-400 bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-200 resize-none">{{ old('deskripsi') }}</textarea>
            </div>
