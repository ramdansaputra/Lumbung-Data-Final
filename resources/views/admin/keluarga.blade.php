...
<div x-data="{
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700" style="overflow:visible">
        ...
        <div class="flex flex-wrap items-center gap-2 px-5 pt-5 pb-4 border-b border-gray-100 dark:border-slate-700">
            ...
            <div class="relative" x-data="{ open: false }" @click.away="open = false">
                ...
                <div x-show="open" x-transition
                    ...
                    <template x-if="selectedIds.length === 0">
                        <span class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-300 dark:text-slate-600 cursor-not-allowed select-none">
                            ...
                            <form method="POST" action="{{ route('admin.keluarga.pindah-wilayah-kolektif') }}"
                                ...
                                <select name="wilayah_id" required
                                    ...
                                    @foreach($wilayahList as $w)
                                        <option value="{{ $w->id }}">{{ $w->dusun }} RT {{ $w->rt }} / RW {{ $w->rw }}</option>
                                    @endforeach
                                </select>
                                <button type="submit"
                    ...
                    <div x-show="open" x-transition class="absolute left-0 top-full mt-1 w-full z-[100] bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg shadow-lg overflow-hidden" style="display:none">
                        <ul class="py-1">
                            <template x-for="opt in options" :key="opt.value">
                                <li @click="choose(opt)" class="px-3 py-2 text-sm cursor-pointer hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors" :class="selected === opt.value ? 'bg-emerald-500 text-white' : 'text-gray-700 dark:text-slate-200'" x-text="opt.label"></li>
                            </template>
                        </ul>
                    </div>
                </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-b border-gray-100 dark:border-slate-700">
            <form method="GET" action="{{ route('admin.keluarga') }}" class="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-32">AKSI</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                    @forelse($keluarga as $index => $kk)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors"
                            ...
                            <td class="px-3 py-3 text-gray-500 dark:text-slate-400 tabular-nums">{{ $keluarga->firstItem() + $index }}</td>

                            {{-- AKSI --}}
                            <td class="px-3 py-3 whitespace-nowrap" style="position:static;overflow:visible">
                                <div class="relative" x-data="{ open: false }" @click.away="open = false" style="position:relative">
                                    <button @click="open = !open"
                            ...
                            <td class="px-3 py-3">
                                ...
                                @else
                                    <div class="w-9 h-9 rounded-full bg-gray-200 dark:bg-slate-600 flex items-center justify-center">
                                        <svg class="w-5 h-5 text-gray-400 dark:text-slate-400" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <td class="px-3 py-3 whitespace-nowrap">
                                ...
                                @if(str_starts_with($kk->no_kk, '0'))
                                    <span class="block mt-0.5 px-1.5 py-0.5 text-[10px] font-semibold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 rounded w-fit">Sementara</span>
                                @endif
                            </td>
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between flex-wrap gap-3">
            <p class="text-sm text-gray-500 dark:text-slate-400">
                @if($keluarga->total() > 0)
                    Menampilkan {{ $keluarga->firstItem() }}–{{ $keluarga->lastItem() }} dari {{ number_format($keluarga->total()) }} entri
                @else
                    Menampilkan 0 entri
                @endif
            </p>
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $keluarga->appends(request()->query())->previousPageUrl() }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 transition-colors">Sebelumnya</a>
                @endif
                @php $cp=$keluarga->currentPage(); $lp=$keluarga->lastPage(); $s=max(1,$cp-2); $e=min($lp,$cp+2); @endphp
                @if($s>1)<a href="{{ $keluarga->appends(request()->query())->url(1) }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 transition-colors">1</a>@if($s>2)<span class="px-1 text-gray-400">…</span>@endif@endif
                @for($p=$s;$p<=$e;$p++)
                    @if($p==$cp)<span class="px-3 py-1.5 text-sm font-semibold text-white bg-emerald-600 border border-emerald-600 rounded-lg">{{ $p }}</span>
                    @else<a href="{{ $keluarga->appends(request()->query())->url($p) }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 transition-colors">{{ $p }}</a>@endif
                @endfor
                @if($e<$lp)@if($e<$lp-1)<span class="px-1 text-gray-400">…</span>@endif<a href="{{ $keluarga->appends(request()->query())->url($lp) }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 transition-colors">{{ $lp }}</a>@endif
                @if($keluarga->hasMorePages())
                    <a href="{{ $keluarga->appends(request()->query())->nextPageUrl() }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 transition-colors">Selanjutnya</a>
                @else
    ...
    <div x-data="{ show: false }"
        @buka-modal-rumah-tangga-kolektif.window="show = true"
        x-show="show"
