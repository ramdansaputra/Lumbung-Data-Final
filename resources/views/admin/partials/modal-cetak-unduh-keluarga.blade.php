...
<div
    x-data="{
        ...
        selectedIds: [],
        get urlCetak() {
            let base = '{{ route('admin.keluarga.export.pdf') }}';
            let q = new URLSearchParams({{ json_encode(request()->query()) }});
            if (this.semuaData) q.set('semua', '1');
            if (this.selectedIds.length > 0) q.set('selected_ids', this.selectedIds.join(','));
            return base + '?' + q.toString();
        }
    }"
    @buka-modal-cetak-keluarga.window="show = true; selectedIds = ($event.detail?.selectedIds ?? [])"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg" @click.stop>
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="rounded-xl bg-emerald-500 p-4 space-y-2">
                    ...
                    <p class="text-sm text-white/90 leading-relaxed" x-show="selectedIds.length > 0">
                        Mode data terpilih aktif untuk <strong class="text-white" x-text="selectedIds.length"></strong> KK.
                    </p>
                </div>

                <div class="flex items-start gap-3 p-3.5 rounded-lg border border-gray-200 dark:border-slate-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors cursor-pointer"
                    ...
                    <div>
                        <p class="text-sm font-semibold text-gray-700 dark:text-slate-200">Cetak Semua Data</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Abaikan paginasi halaman saat ini.</p>
                    </div>
                </div>
            </div>
...
<div
    x-data="{
        ...
        selectedIds: [],
        get urlUnduh() {
            let base = '{{ route('admin.keluarga.export.excel') }}';
            let q = new URLSearchParams({{ json_encode(request()->query()) }});
            if (this.semuaData) q.set('semua', '1');
            if (this.selectedIds.length > 0) q.set('selected_ids', this.selectedIds.join(','));
            return base + '?' + q.toString();
    ...
    @buka-modal-unduh-keluarga.window="show = true; selectedIds = ($event.detail?.selectedIds ?? [])"
    @keydown.escape.window="show && (show = false)"
>
    <div x-show="show" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-150"
    ...
    <div x-show="show" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg" @click.stop>
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="rounded-xl bg-emerald-500 p-4 space-y-2">
                    <p class="text-sm text-white/90 leading-relaxed">
                        Gunakan opsi ini untuk mengunduh data sesuai filter aktif atau data terpilih.
                    </p>
                    <p class="text-sm text-white/90 leading-relaxed" x-show="selectedIds.length > 0">
                        Mode data terpilih aktif untuk <strong class="text-white" x-text="selectedIds.length"></strong> KK.
                    </p>
                ...
                <div class="flex items-start gap-3 p-3.5 rounded-lg border border-gray-200 dark:border-slate-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors cursor-pointer"
                    ...
                    <div>
                        <p class="text-sm font-semibold text-gray-700 dark:text-slate-200">Unduh Semua Data</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Abaikan paginasi halaman saat ini.</p>
                    </div>
                </div>
            </div>
