...
═══════════════════════════════════════════════════════ --}}
    <div class="flex items-center justify-between mb-5">
        <div>
            ...
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Laporan perkembangan penduduk (Lampiran A-9) per
                bulan</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <form method="GET" id="form-filter-laporan" class="flex flex-wrap gap-4 mb-6 items-end">
            ...
            <div x-data="{
                ...
                <label
                    class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">Tahun</label>
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="p-2 border-b border-gray-100 dark:border-slate-700">
                        <input type="text" x-model="search" @keydown.escape="open = false" placeholder="Cari tahun..."
            ...
            <div x-data="{
                ...
                options: [
                    ...
                    {value:'{{ $num }}', label:'{{ $nama }}'}, @endforeach
                ],
                ...
                <label
                    class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wide">Bulan</label>
                <button type="button" @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-2 border rounded-lg text-sm cursor-pointer bg-white dark:bg-slate-700 focus:outline-none transition-colors"
                    :class="open ? 'border-emerald-500 ring-2 ring-emerald-500/20' :
                        'border-gray-300 dark:border-slate-600 hover:border-emerald-400'">
        ...
        <div class="text-center mb-4">
            <p class="font-bold text-base uppercase">
                ...
                {{ strtoupper($data['identitas']->kabupaten ?? ($data['identitas']->nama_kabupaten ?? '')) }}
            </p>
                ...
                <tbody>
                    @foreach ($data['rows'] as $row)
                        @php
                            ...
                            $rowClass = $isHighlight ? 'bg-slate-50 font-medium' : 'hover:bg-gray-50';
...
═══════════════════════════════════════════════════════ --}}
    <div id="cetakModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div x-data="{
                    ...
                    options: [
                        ...
                        {value:'{{ $p->id }}', label:'{{ addslashes($p->nama) }} ({{ addslashes($p->jabatan ?? '-') }})'}, @endforeach
                    ],
                    get filtered() { return !this.search ? this.options : this.options.filter(o => o.label.toLowerCase().includes(this.search.toLowerCase())); },
                    choose(opt) {
                        this.selected = opt.value;
...
═══════════════════════════════════════════════════════ --}}
    <div id="unduhModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg">
    ...
    <script>
        ...
        function doCetak() {
            ...
            const url =
                `{{ url()->current() }}?month={{ $data['selectedMonth'] }}&year={{ $data['selectedYear'] }}&action=print&ttd=${ttd}&diketahui=${diketahui}`;
        ...
        function doUnduh() {
            ...
            const url =
                `{{ url()->current() }}?month={{ $data['selectedMonth'] }}&year={{ $data['selectedYear'] }}&download=excel&ttd=${ttd}&diketahui=${diketahui}`;
