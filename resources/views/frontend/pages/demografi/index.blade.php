...
<section class="pt-16 pb-12 bg-slate-50 relative min-h-screen z-10">
    <div class="container mx-auto px-4 max-w-7xl">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
            ...
            <div class="bg-gradient-to-br from-[#059669] to-[#047857] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white shadow-md hover:-translate-y-1 transition duration-300">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-2xl md:text-4xl lg:text-5xl font-extrabold mb-1 leading-none">{{ number_format($totalPenduduk, 0, ',', '.') }}</h3>
            ...
            <div class="bg-gradient-to-br from-[#3b82f6] to-[#2563eb] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white shadow-md hover:-translate-y-1 transition duration-300">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        <p class="text-blue-100 text-[10px] md:text-sm font-semibold uppercase tracking-wider mb-1 md:mb-2">Kepala Keluarga</p>
                        <h3 class="text-2xl md:text-4xl lg:text-5xl font-extrabold mb-1 leading-none">{{ number_format($totalKeluarga, 0, ',', '.') }}</h3>
            ...
            <div class="bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white shadow-md hover:-translate-y-1 transition duration-300">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
            ...
            <div class="bg-gradient-to-br from-[#f59e0b] to-[#d97706] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white shadow-md hover:-translate-y-1 transition duration-300">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
        ...
        {{-- JENIS KELAMIN --}}
        <div class="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-slate-100 mb-6 md:mb-8">
            ...
            <div class="w-full h-3 md:h-4 rounded-full flex overflow-hidden bg-slate-100">
                <div class="bg-[#0ea5e9] h-full transition-all duration-1000" style="width: {{ $persenLaki }}%"></div>
                <div class="bg-[#f43f5e] h-full transition-all duration-1000" style="width: {{ $persenPerempuan }}%"></div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-8 mb-8">
            ...
            {{-- Distribusi Usia --}}
            <div class="bg-white rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                    ...
                    @foreach($usiaData as $item)
                        <div>
                            ...
                            <div class="w-full bg-slate-100 rounded-full h-2">
                                <div class="bg-[#a855f7] h-2 rounded-full" style="width: {{ $item['persen'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                ...
                <div class="p-5 md:p-6 flex flex-col space-y-4 md:space-y-5 max-h-80 overflow-y-auto custom-scrollbar">
                    @foreach($pendidikanData as $item)
                        <div>
                            ...
                            <div class="w-full bg-slate-100 rounded-full h-2">
                                <div class="bg-[#10b981] h-2 rounded-full" style="width: {{ $item['persen'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="p-4 md:p-6 bg-slate-50/50">
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4">
                        @foreach(array_slice($pekerjaanData, 0, 6) as $item)
                            <div class="bg-white p-3 md:p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition">
                                <p class="text-2xl md:text-3xl font-extrabold text-[#f97316] mb-1 leading-none">{{ $item['persen'] }}<span class="text-sm md:text-lg">%</span></p>
                    ...
                    @foreach($agamaData as $index => $item)
                        ...
                        <div>
                            ...
                            <div class="w-full bg-slate-100 rounded-full h-1.5">
                                <div class="{{ $color }} h-1.5 rounded-full" style="width: {{ $item['persen'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
