...
@section('content')
    ...
    <section class="py-16 bg-gray-50">
        <div class="container mx-auto px-4">
            ...
            <div class="flex flex-col lg:flex-row gap-12">
                ...
                {{-- ===== KOLOM KIRI: Produk + Ulasan ===== --}}
                <div class="lg:w-2/3 min-w-0">

                    {{-- Header Info Toko (Mobile only) --}}
                    ...
                    @if($produk->count() > 0)
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                            @foreach($produk as $item)
                                <div class="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md hover:border-emerald-100 transition-all duration-300">
                                    ...
                                    {{-- Info Produk --}}
                                    <div class="p-3">
                                        <h3 class="text-sm font-semibold text-gray-800 line-clamp-2 mb-1 group-hover:text-emerald-600 transition">
                                            {{ $item->nama_produk }}
                                        </h3>
                    ...
                    @else
                        <div class="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-gray-200 mb-8">
                    ...
                    <div class="border-t border-gray-200 pt-10">
                        ...
                        {{-- Ringkasan Rating --}}
                        @if($ulasan->count() > 0)
                            @php $avgRating = round($ulasan->avg('rating'), 1); @endphp
                            <div class="flex flex-col sm:flex-row sm:items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
                                <div class="text-center sm:border-r sm:border-gray-100 sm:pr-6">
                                    <p class="text-4xl font-extrabold text-gray-900">{{ $avgRating }}</p>
                                    <div class="flex justify-center gap-0.5 my-1">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="{{ $i <= round($avgRating) ? 'fa-solid' : 'fa-regular' }} fa-star text-lg {{ $i <= round($avgRating) ? 'text-yellow-400' : 'text-gray-300' }}"></i>
                                        @endfor
                                    </div>
                        ...
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-lg font-bold text-gray-900">
                                Ulasan Pembeli
                                @if($ulasan->count() > 0)
                                    <span class="ml-1.5 text-sm font-medium text-gray-400">({{ $ulasan->count() }})</span>
                                @endif
                            </h3>
                        </div>
                                    ...
                                    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                        <div class="flex items-start justify-between gap-3 mb-2">
                                            <div class="flex items-center gap-3">
                                                <div class="w-9 h-9 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-sm flex-shrink-0">
                                                    {{ strtoupper(substr($review->nama, 0, 1)) }}
                                                </div>
                                                <div>
                                                    <p class="font-semibold text-gray-800 text-sm leading-tight">{{ $review->nama }}</p>
                                                    <div class="flex items-center gap-0.5 mt-0.5">
                                                        @for($i = 1; $i <= 5; $i++)
                                                            <i class="{{ $i <= $review->rating ? 'fa-solid' : 'fa-regular' }} fa-star text-sm {{ $i <= $review->rating ? 'text-yellow-400' : 'text-gray-300' }}"></i>
                                                        @endfor
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <p class="text-sm text-gray-600 leading-relaxed pl-12">
                                            {{ $review->komentar }}
                                        </p>
                                    </div>
                                @endforeach
                        ...
                        @else
                            <div class="text-center py-10 bg-white rounded-2xl border border-dashed border-gray-200 mb-8">
                                <i class="fa-regular fa-comments text-4xl text-gray-300 mb-3 block"></i>
                                <p class="text-sm font-medium text-gray-500">Belum ada ulasan. Jadilah yang pertama!</p>
                            </div>
                        @endif
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                            ...
                            @if(session('success_ulasan'))
                                <div class="mb-4 px-4 py-3 bg-emerald-50 text-emerald-700 text-sm rounded-xl border border-emerald-100 flex items-center gap-2">
                                    <i class="fa-solid fa-circle-check flex-shrink-0"></i>
                                    {{ session('success_ulasan') }}
                                </div>
                            @endif

                            <form method="POST" action="{{ route('ulasan.store', $lapak->id) }}" class="space-y-4">
                                @csrf

                                {{-- Nama --}}
                                <div>
                                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Nama Anda</label>
                                    <input type="text" name="nama"
                                        placeholder="Masukkan nama Anda"
                                        value="{{ old('nama') }}"
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 transition @error('nama') border-red-400 @enderror"
                                        required>
                                    @error('nama')
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2" id="star-rating">
                                        {{-- FA6: style prefix wajib di depan (fa-regular / fa-solid), lalu fa-star --}}
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="fa-regular fa-star star-icon"
                                               data-value="{{ $i }}"
                                               style="font-size:2rem; color:#d1d5db; cursor:pointer; transition: color 0.15s, transform 0.1s;">
                                            </i>
                                        @endfor
                                    </div>
                                    <p class="text-xs text-gray-400 mt-1.5" id="star-label">Pilih rating</p>
                                ...
                                {{-- Komentar --}}
                                <div>
                                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Komentar</label>
                                    <textarea name="komentar" rows="4"
                                        placeholder="Ceritakan pengalaman Anda berbelanja di sini..."
                                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 transition resize-none @error('komentar') border-red-400 @enderror"
                                ...
                                <button type="submit"
                                    class="w-full sm:w-auto px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-xl transition shadow-sm active:scale-95">
                                    Kirim Ulasan
                                </button>
                            </form>
                        </div>
                ...
                {{-- ===== KOLOM KANAN: Sidebar Info Toko ===== --}}
                <div class="lg:w-1/3">
                    <div class="hidden lg:block bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Fungsi ubah tampilan bintang
            function paint(count) {
                stars.forEach(function (s) {
                    const v = parseInt(s.getAttribute('data-value'));
                    if (v <= count) {
                        // Bintang terisi: fa-solid fa-star + kuning
                        s.className = 'fa-solid fa-star star-icon';
                        s.style.color = '#facc15';
                        s.style.transform = 'scale(1.15)';
                    } else {
                        // Bintang kosong: fa-regular fa-star + abu
                        s.className = 'fa-regular fa-star star-icon';
                        s.style.color = '#d1d5db';
                        s.style.transform = 'scale(1)';
                    }
                });
                label.textContent = count > 0 ? labels[count] : 'Pilih rating';
            }

            // Restore nilai lama jika ada (setelah validasi gagal)
            var saved = parseInt(input.value) || 0;
            if (saved > 0) paint(saved);

            // Event: klik → simpan nilai permanen
            stars.forEach(function (s) {
            ...
            // Event: hover → preview sementara
            stars.forEach(function (s) {
                s.addEventListener('mouseenter', function () {
                    paint(parseInt(this.getAttribute('data-value')));
                });
            });

            // Event: mouse pergi dari container → kembalikan ke nilai tersimpan
